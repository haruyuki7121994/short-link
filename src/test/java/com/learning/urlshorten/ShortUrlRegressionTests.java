...
class ShortUrlRegressionTests {
    ...
    ShortUrlRegressionTests() {
        when(redis.opsForValue()).thenReturn(values);
        props.setDomain("https://short.example");
    }
    ...
    @Test
    void redisReadFailureSkipsWriteButReturnsDatabaseMapping() {
    ...
    @Test
    void cacheWriteFailureDoesNotFailRedirect() {
        when(db.findById(code)).thenReturn(Optional.of(entity(null)));
    ...
    @Test
    void writesDurationTtlAndStoresExpiration() {
    ...
    @Test
    void longLivedMappingUsesBoundedCacheTtl() {
    ...
    @Test
    void expiredDatabaseEntryReturns404WithoutCaching() throws Exception {
    ...
    @Test
    void expiredCacheHitDoesNotRedirect() {
    ...
    @Test
    void cacheHitReturnsLocationWithoutDatabaseLookup() throws Exception {
    ...
    @Test
    void malformedCacheFallsBackAndIsReplaced() {
    ...
    @Test
    void unknownMappingReturns404() throws Exception {
        when(db.findById(code)).thenReturn(Optional.empty());
        mvc().perform(get("/api/v1/base62-encode/" + code)).andExpect(status().isNotFound());
    }

    @Test
    void databaseFailureReturns503WithoutLeakingMessage() throws Exception {
    ...
    @Test
    void createWithoutAliasReturns201() throws Exception {
    ...
    @Test
    void aliasCollisionReturns409() throws Exception {
    ...
    @Test
    void randomCollisionRetriesAndPreservesExpiration() {
        when(db.insert(any(ShortUrlEntity.class))).thenThrow(new DuplicateKeyException("duplicate"))
    ...
    @Test
    void validatesDestinationAndOptionalAlias() {
    ...
    @Test
    void concurrentCreatesCannotBothClaimAlias() throws Exception {
    ...
    @Test
    void overloadFailsFastAndReleasesLookupPermit() throws Exception {
        var limited = new Base62EncodeRedirectShortUrlHandler(db, cache, 1);
        var started = new CountDownLatch(1);
        var release = new CountDownLatch(1);
        when(db.findById(code)).thenAnswer(invocation -> {
            started.countDown();
            release.await(2, TimeUnit.SECONDS);
            return Optional.of(entity(null));
        });
